            ...
            if problemchars.search(tag.attrib["k"])==None:
                if tag.attrib["k"].count(":")<2:
                    if tag.attrib["k"].startswith("addr:"):
                        new_tag_name= tag.attrib["k"][tag.attrib["k"].index(":")+1:]
                        address[new_tag_name]=tag.attrib["v"]
                    elif tag.attrib["k"].count(":")==1:
                        new_tag_name= tag.attrib["k"][tag.attrib["k"].index(":")+1:]
                        node[new_tag_name]=tag.attrib["v"]
                    else:
                        node[tag.attrib["k"]]=tag.attrib["v"]
        if len(address)!=0: node["address"]=address
        #3
        if element.tag=="way":
            node_refs=[]
